...
var day = d3.time.format("%w"),
    ...
    formatTime = d3.time.format("%d-%m-%Y");

var color = d3.scale.linear().range(["white", '#002b53'])
    .domain([0, 1])
    
var svg = d3.select(".calender-map").selectAll("svg")
    .data(d3.range(1938, 2019))
  .enter().append("svg")
    .attr("width", '100%')
...
d3.csv("transformed_msp_precip.csv", function(error, csv) {

  csv.forEach(function(d) {
    d.precip = Number(d.precip);
  });
...
var data = d3.nest()
    ...
    .rollup(function(d) { return  Math.sqrt(d[0].precip / (precip_max - 5)); })
    .map(csv);
